...
class MixtureDiscreteEulerSolver(Solver):
        ...
        **model_extras,
        ...
        with ctx:
            for i in range(n_steps):
                if 'step_size_list' in model_extras:
                    step_size_list = model_extras.get("step_size_list")
                    model_extras['step_size_model'] = step_size_list[:, i]
    ...
    def sample_v2(
        ...
        **model_extras,
        ...
        with ctx:
            for i in range(n_steps):
                ...
                if i == n_steps - 1:
                    ...
                    jump_count = jump_count + mask_jump.sum()
                    print(mask_jump.sum())
        ...
        if return_intermediates:
            if step_size is None:
                return torch.stack(res, dim=0), jump_count
            else:
                return torch.stack(res, dim=0)[order], jump_count
        else:
            return x_t, jump_count
    ...
    def sample_masked(
        ...
        **model_extras,
        ...
        """
        Like `sample`, but only updates tokens marked editable. Non-editable tokens
        remain equal to their initial values throughout the process.

        Args:
            x_init: [B, ...] integer tensor of initial tokens.
            step_size: uniform step size if not None; otherwise respects `time_grid`.
            mask_token_id: integer token id considered "masked" (editable). Optional if `edit_mask` is provided.
            edit_mask: boolean mask with same shape as x_init; True = editable. If provided, overrides `mask_token_id`.
            div_free, dtype_categorical, time_grid, return_intermediates, verbose, **model_extras:
                Same semantics as in `sample`.

        Returns:
            Tensor with the same shape as x_init (or stacked intermediates if `return_intermediates=True`).
        """
        if not div_free == 0.0:
            assert self.source_distribution_p is not None, (
                "Source distribution p must be specified to add a divergence-free term."
            )
        ...
        if edit_mask is None:
            assert mask_token_id is not None, "Provide either edit_mask or mask_token_id."
            editable = (x_init == mask_token_id)
        else:
            editable = edit_mask.to(dtype=torch.bool, device=x_init.device)
            assert editable.shape == x_init.shape, "edit_mask must match shape of x_init."

        # If nothing is editable, fast-path return
        if not editable.any():
            if return_intermediates:
                return torch.stack([x_init.clone()], dim=0)
            return x_init.clone()

        # Time discretization (same as `sample`)
        if step_size is None:
            t_discretization = time_grid
            n_steps = len(time_grid) - 1
            t_final = time_grid[-1].item()
        ...
        with ctx:
            for i in range(n_steps):
                ...
                # Force non-editable positions to keep their current tokens
                # so that one-hots reflect a no-op on those coordinates.
                x_1 = torch.where(editable, x_1, x_t)

                if i == n_steps - 1:
                    # At the final step, collapse to x_1 but only for editable coords.
                    x_t = torch.where(editable, x_1, x_t)
                else:
                    # Compute velocity u_t(·|x_t, x_1) as usual
                    scheduler_output = self.path.scheduler(t=t)
                    k_t = scheduler_output.alpha_t
                    d_k_t = scheduler_output.d_alpha_t
                    ...
                    # Divergence-free add-on (optional)
                    div_free_t = div_free(t) if callable(div_free) else div_free
                    if div_free_t > 0:
                        p_0 = self.source_distribution_p[(None,) * x_t.dim()]
                        u = u + div_free_t * d_k_t / (k_t * (1 - k_t)) * (
                    ...
                    # Zero the self-transition as usual
                    delta_t = F.one_hot(x_t, num_classes=self.vocabulary_size)
                    u = torch.where(delta_t.to(dtype=torch.bool), torch.zeros_like(u), u)

                    # === Mask out non-editable positions entirely ===
                    # Make sure the rate is zero outside editable coords:
                    u = u * editable[..., None]

                    # Poisson jump decision
                    intensity = u.sum(dim=-1)  # zero where not editable
                    mask_jump = torch.rand(size=x_t.shape, device=x_t.device) < (1 - torch.exp(-h * intensity))

                    # Only editable positions can jump (already enforced by intensity==0),
                    # but keep the logical constraint explicit:
                    mask_jump = mask_jump & editable
