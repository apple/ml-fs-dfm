...
from fs_dfm.utils.metrics import log_metrics
...
class TrainLogger:
    ...
    def log_metric(self, value: float, name: str, stage: bool, step: int) -> None:
        ...
        if self.is_main_process():
            log_metrics({f"{name}": value}, step=step)
        if self.enable_wandb:
            self.wandb_logger.log(data={f"{stage}/{name}": value}, step=step)

    def log_lr(self, value: float, step: int) -> None:
        if self.is_main_process():
            log_metrics({"Optimization/LR": value}, step=step)
        if self.enable_wandb:
            self.wandb_logger.log(data={"Optimization/LR": value}, step=step)

    def log_lr_teacher(self, value: float, step: int) -> None:
        if self.is_main_process():
            log_metrics({"Optimization/LR Teache": value}, step=step)
        if self.enable_wandb:
            self.wandb_logger.log(data={"Optimization/LR Teache": value}, step=step)
