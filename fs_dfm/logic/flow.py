...
def get_loss_function(
    loss_function: str, path: Optional[ProbPath] = None, temperature: float = 1.0
) -> _Loss:
    ...
    elif loss_function == "adaptive_ce_kl":
        return AdaptiveCEKLLoss(
            path=path, temperature=temperature, gamma=1.0, reduction="none"
        )
    elif loss_function == "KLDistillation":
        return ForwardKLDistillationLoss(tau=temperature)
    else:
