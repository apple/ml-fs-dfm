...
class DDiTBlock(nn.Module):
    """
    Transformer block with DDiT to compute attention and MLP
    """
    ...
    def __init__(
        ...
        self.dt_conditioned = dt_conditioned
        if dt_conditioned:
            self.condition_fuser = nn.Linear(2 * cond_dim, cond_dim)
    ...
    def forward(self, x: Tensor, rotary_cos_sin: Tensor, c: Tensor) -> Tensor:
        ...
        if self.dt_conditioned:
            c = F.silu(self.condition_fuser(c))
...
class DDitFinalLayer(nn.Module):
    ...
    def __init__(
        self, hidden_size: int, out_channels: int, cond_dim: int, dt_conditioned: bool
    ):
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(
        ...
        if dt_conditioned:
            self.dt_embedding = TimestepEmbedder(hidden_size=config.cond_dim)
            self.time_condition_fuser = nn.Linear(
                2 * config.cond_dim, 2 * config.cond_dim
            )
    ...
    def forward(self, x_t: Tensor, time: Tensor, dt: Optional[Tensor] = None) -> Tensor:
        x = self.vocab_embed(x_t)
        t_emb = self.time_embedding(time)

        if self.dt_conditioned:
            dt_emb = self.dt_embedding(dt)
            fused = torch.cat([t_emb, dt_emb], dim=-1)  # [B, 2 * cond_dim]
            c = F.silu(self.time_condition_fuser(fused))
        else:
            c = F.silu(t_emb)
